...
export default function BookingScreen({ route, navigation, user }) {
  ...
  const handleConfirm = async () => {
    if (!selectedSlot) {
      Alert.alert('Pick a time', 'Please select an available time slot.');
      return;
    }
    setConfirming(true);
    const booking = await createBooking({
      ...
      date: `${DAYS[selectedDay]}, May ${DATES[selectedDay]}`,
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.successContainer}>
          ...
          <Text style={styles.successSub}>Your slot has been reserved at</Text>
          ...
          <View style={styles.confirmCard}>
            <View style={styles.confirmRow}>
              ...
              <Text style={styles.confirmValue}>{service?.name || 'General Service'}</Text>
            </View>
            <View style={styles.confirmRow}>
              <Ionicons name="calendar-outline" size={16} color="#888" />
              <Text style={styles.confirmLabel}>Date</Text>
              <Text style={styles.confirmValue}>{DAYS[selectedDay]}, May {DATES[selectedDay]}</Text>
            </View>
            <View style={styles.confirmRow}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.serviceSelected}>
            <View>
              <Text style={styles.serviceSelectedName}>{service?.name || 'General Service'}</Text>
      ...
      <View style={styles.confirmBar}>
        <View>
          <Text style={styles.confirmBarLabel}>
            {selectedSlot ? `${DAYS[selectedDay]}, May ${DATES[selectedDay]} at ${selectedSlot}` : 'No slot selected'}
          </Text>
